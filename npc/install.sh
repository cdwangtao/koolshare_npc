...
install_now(){
  ...
  find /koolshare/init.d -name "*npc*" | xargs rm -rf >/dev/null 2>&1
  ...
  # 如果配置不存在 那么设置默认值
  if [ "$(dbus get ${module}_common_server_ip)" == "" ];then
    dbus set ${module}_common_server_ip="127.0.0.1"
  fi
  if [ "$(dbus get ${module}_common_server_port)" == "" ];then
    dbus set ${module}_common_server_port="85"
  fi
  if [ "$(dbus get ${module}npc_common_vkey)" == "" ];then
    dbus set ${module}npc_common_vkey="vkey"
  fi
  ...
  if [ "${ENABLE}" == "1" -a -f "/koolshare/scripts/${module}_config.sh" ];then
    ...
    sh /koolshare/scripts/npc_config.sh restart
  fi
